...
export LIBRARY_NAME="${LIBRARY_NAME:-biblioteca}"
...
update_koha_database_conf () {
    ...
    envsubst < /templates/koha-common.cnf > /etc/mysql/koha-common.cnf
}
...
update_apache2_conf () {
    ...
    then
        ...
        envsubst < /templates/koha.conf > /etc/apache2/sites-available/${LIBRARY_NAME}.conf
    else 
        ...
        envsubst < /templates/koha-no-domain.conf > /etc/apache2/sites-available/${LIBRARY_NAME}.conf
    fi
